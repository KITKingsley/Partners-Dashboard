(function () {
  ...
  if (els.uploadStripeButton && els.stripeFileInput) {
  ...
  window.DashboardApp = {
    ...
    searchIndex(query) {
      ...
      for (const row of rows) {
        if (!rowSearchText(row).includes(q)) continue;

        if (!orgSeen.has(row.organization)) {
          orgSeen.add(row.organization);
          results.push({
            type: "organization",
            title: row.organization,
            subtitle: `${row.platform} partner`,
            organization: row.organization
          });
        }
      ...
      for (const row of rows) {
        ...
        results.push({
          type: "transaction",
          title: `${row.organization} · ${row.dateLabel}`,
          subtitle: `${row.platform} · ${money.format(row.totalBeforeGst)}`,
          organization: row.organization,
    ...
    openResult(item) {
      ...
      if (item.organization) {
        state.organization = item.organization;
        if (els.organizationFilter) els.organizationFilter.value = item.organization;
      }
      if (item.platform) {
        state.platform = item.platform;
        if (els.platformFilter) els.platformFilter.value = item.platform;
      }
      render();
    }
  };
